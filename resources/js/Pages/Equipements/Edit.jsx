...
export default function Edit({ equipement, sites, types }) {
    ...
    const submit = (e) => {
        e.preventDefault();
        put(route('equipements.update', equipement.id));
    };

    return (
        <AuthenticatedLayout>
            <Head title={`Modifier | ASSET MGMT`} />
            ...
            <div className="max-w-4xl mx-auto space-y-8">
                <div className="flex items-center justify-between">
                    <div className="flex items-center gap-4">
                        ...
                        <div>
                            <h2 className="text-3xl font-display font-extrabold text-white">Modifier l'Actif</h2>
                            <p className="text-gray-500 mt-1">Mettez à jour les informations de {equipement.nom}.</p>
                        </div>
                    </div>
                </div>
                ...
                >
                    <form onSubmit={submit} className="space-y-6">
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                            ...
                            <div className="space-y-2">
                                ...
                                <select
                                    ...
                                    className="w-full bg-white/[0.03] border border-white/10 rounded-xl py-3 px-4 text-white focus:outline-none focus:border-ocp-green transition-all appearance-none"
                                >
                                    <option value="actif" className="bg-ocp-navy">Actif</option>
                                    <option value="en maintenance" className="bg-ocp-navy">En Maintenance</option>
                                    <option value="hors service" className="bg-ocp-navy">Hors Service</option>
                                </select>
                                {errors.statut && <p className="text-red-400 text-xs mt-1">{errors.statut}</p>}
                            </div>
                        </div>
                        ...
                        <div className="pt-6 border-t border-white/5 flex justify-end gap-4">
                            ...
                            >
                                <Save size={18} />
                                {processing ? 'Mise à jour...' : 'Mettre à jour l\'actif'}
                            </button>
                        </div>
                    </form>
                </motion.div>
